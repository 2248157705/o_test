<template>
  <global-view ref="global" bgColor="#F7F9FA">
    ...
    <view class="sale-form">
      <view class="sf-item">
        <view class="title"><text>1、选择售后原因</text></view>
    ...
    <Popup :show="showPopup" title="请选择投诉原因" @close="showPopup = false">
      <view
        v-for="(item, index) in afterSaleApplyReasonOptions"
        :key="index"
        class="popup-item"
      >
        <u-checkbox-group
...
import { afterSaleApply } from "@/api/order";
import Popup from "@/components/popup/index.vue";
import { afterSaleApplyReasonOptions } from "@/pages/order/js/order";
...
const confirm = () => {
  ...
  const data =
    afterSaleApplyReasonOptions[checkedAgree.value.findIndex((value) => value)];
  ...
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      for (let i = 0; i < count; i++) {
        getCosAuthJava({
          type: "idCard",
          source_file_name_list: [res.tempFiles[i].name],
        }).then((cos) => {
          const data = cos[0];
          upload(
            res.tempFilePaths[i],
