...
import { getProductInfo, getEstimateProductInfo } from "@/api/product";
...
const paramsData = reactive({
  enterType: ProductTypeEnum.ACCOUNT,
  ticketId: "1",
});
...
const navBarTitle = computed(() => {
  ...
  if (paramsData.enterType == ProductTypeEnum.RECYCLE) {
    return "闪电回收";
  ...
  form.value.validate(async (state, submitData, params) => {
    ...
    if (
      ...
      if (submitData.screenshot_info) {
        submitData.screenshot_info = submitData.screenshot_info.map((item) => {
          item.url = uniqueString(item.url);
          return item;
        });
    ...
    submitData.productId = formFileds.value.product_id;
    ...
    if (estimate_id || product_id) {
      ...
      if (
        ...
        // });
        let delta = 2;
        if (ProductTypeEnum.RECYCLE === paramsData.enterType) {
          delta = 1;
        }
        console.log(
          "dsfdfdfd1234339999999:",
          paramsData.enterType,
          ProductTypeEnum.RECYCLE,
          delta
        );
        uni.navigateBack({
          delta,
        ...
        formFileds.value = {
          user_QQ: submitData.user_QQ,
          user_phone: submitData.user_phone,
          user_wechat: submitData.user_wechat,
        };
      }
    }
  });
...
onLoad(async (param) => {
  paramsData.enterType = param.type - 0;
  paramsData.ticketId = param.ticketId == "undefined" ? "1" : param.ticketId;

  if (param.edit_product_id) {
    const data = await getProductInfo({ productId: param.edit_product_id });
    param.formFileds = data;
  }
  if (param.estimate_id) {
    const { data } = await getEstimateProductInfo(param.estimate_id);
    param.formFileds = data;
  }

  if (param.formFileds) {
    const { goods_info, screenshot_info, user_qq, user_QQ, ...data } =
      param.formFileds;
    const info =
      typeof goods_info === "string" ? JSON.parse(goods_info) : goods_info;
    formFileds.value = {
      ...data,
      ...info,
      screenshot_info: screenshot_info ? JSON.parse(screenshot_info) : [],
      user_QQ: user_qq || user_QQ,
    };
  }
  if (
    [
      ...
      ProductTypeEnum.RECYCLE,
      ProductTypeEnum.GUARANTEE,
    ].includes(param.type - 0)
  ) {
    const status = {
      1: RateConfigStatus.SERVICE_CHARGE,
      3: RateConfigStatus.GUARANTEE,
      5: RateConfigStatus.RECYCLING,
    };
    const data = await getProductRule({
      uid: userStore.userInfo.uid,
      biz_type: status[param.type],
    });
    costData.value = data;
  }
});
