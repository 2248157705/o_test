...
function handleQuery(callback) {
	try {
		setTimeout(function () {
			_getApp().zp_handleQueryCallback = callback;
		}, 1);
	} catch (e) { }
}
...
function _handleQuery(pageNo, pageSize, from, lastItem) {
	const callback = _getApp().zp_handleQueryCallback;
...
function handleLanguage2Local(callback) {
	try {
		setTimeout(function () {
			_getApp().zp_handleLanguage2LocalCallback = callback;
		}, 1);
	} catch (e) { }
}
...
function _handleLanguage2Local(language, local) {
	const callback = _getApp().zp_handleLanguage2LocalCallback;
