...
export default {
  ...
  methods: {
    ...
    getIndexListLetter(pageY) {
      ...
      if (this.sys.platform == "android") {
        pageY += top;
      }
      // #ifdef H5
      pageY += this.sys.windowTop;
      // #endif
    ...
    setValueForTouch(currentIndex) {
      ...
      console.log(this.anchors[currentIndex].$refs[anchor]);
      this.anchors[currentIndex] &&
    ...
    async scrollHandler(e) {
      ...
      children = this.children.map((item, index) => {
        const height = this.children[1].top - this.children[0].top;
        const child = {
          height,
          top,
        };
