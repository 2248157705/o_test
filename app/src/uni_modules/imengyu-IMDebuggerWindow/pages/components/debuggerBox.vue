<template>
  <view
    :class="'debugger-box' + (isFromWindow ? ' window' : '')"
  >
    <view v-if="!isFromWindow" class="debugger-title">
      ...
      <view class="tool">
        <button
          ...
          @touchmove="onDraggingHeight"
          @touchend="isDragging = false"
        >
          调整高度
        </button>
        <button class="close tool-button" @click="handleClose">关闭</button>
      </view>
    </view>
    <view :class="'debugger-tab' + (isFromWindow ? ' full' : '')">
      ...
      <view class="debugger-tab-area">
        ...
        >
          <view class="head-area">
            <button class="tool-button" @click="onClearNetworkLogs">
              清空
            </button>
            <text class="head-text" style="margin-left: 8rpx"
        ...
        >
          ...
          <view
            ...
            :class="'console-item ' + item.type"
          >
            <text class="time" @longpress="onCopy(item.str)">{{
              item.key
            }}</text>
    ...
    onCloseToolBar(forever) {
      if (forever) {
        uni.showModal({
          title: "提示",
          content:
            "是否要隐藏调试悬浮窗? 永久隐藏后，可以卸载再安装以显示悬浮窗",
          success(res) {
            if (res.confirm) {
